    ...
    versions: [
      {
        ...
        content: "> [!TIP]\n> **AI Security Benchmark Executive Takeaway:**\n> * **Benchmark Release**: Meta AI CyberSecEval 3 & MITRE Engenuity Cyber AI Bench\n> * **#1 Overall Performer**: Claude 3.7 Sonnet (94.2 Overall Security Index)"
      }
    ]
  }
];
...
function getDB() {
  ...
  try {
    ...
    // Ensure all seed published articles exist in db if missing and populate slugs
    for (const seed of SEED_PUBLISHED_ARTICLES) {
      const existing = parsed.publishedArticles.find(a => a.id === seed.id);
      if (!existing) {
        parsed.publishedArticles.push(seed);
...
export function getPublishedArticleById(idOrSlug) {
  ...
  const db = getDB();
  const rawTarget = decodeURIComponent(idOrSlug).trim();
  const target = rawTarget.toLowerCase();
  const targetClean = target.replace(/[^a-z0-9]/g, '');

  // 1. Direct ID match
  let art = db.publishedArticles.find(a => a.id.toLowerCase() === target);
  ...
  if (!art) {
    art = db.publishedArticles.find(a => {
      ...
      return artSlugClean === targetClean || targetClean.startsWith(artSlugClean) || artSlugClean.startsWith(targetClean);
    });
  }
  ...
  if (!art) {
    art = db.publishedArticles.find(a => {
      ...
      return computedSlug === target || target.startsWith(computedSlug) || computedSlug.startsWith(target);
    });
  }
  ...
  if (!art) {
    art = db.publishedArticles.find(a => target.endsWith(a.id.toLowerCase()) || target.includes(a.id.toLowerCase()));
  }
...
export function addPublishedArticle(article) {
  const db = getDB();
  const slug = article.slug || slugify(article.title) || `advisory-${Date.now()}`;
  const imageUrl = article.imageUrl || resolveArticleThumbnail(article.category, article.title);
  const newArticle = {
    id: `pub-${Date.now()}`,
    slug,
    views: 0,
    ...
    ...article,
    imageUrl // guarantee imageUrl is preserved
  };
