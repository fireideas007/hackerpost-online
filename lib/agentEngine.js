...
import { verifyAdvisoryLegitimacy } from './feedLegitimacyVerifier.js';
...
export async function runAgentCycle(triggerSource = "manual", options = {}) {
  ...
  try {
    ...
    // Balance selection: guarantee both cybersecurity advisories and tech startup stories
    const startupItems = prioritized.filter(i => i.category === "SecTech & Startups" || i.category === "M&A & Funding");
    const threatItems = prioritized.filter(i => i.category !== "SecTech & Startups" && i.category !== "M&A & Funding");

    let itemsToProcess = [];
    if (startupItems.length > 0 && threatItems.length > 0) {
      const threatCount = Math.max(1, Math.ceil(batchSize / 2));
      const startupCount = Math.max(1, batchSize - threatCount);
      itemsToProcess = [...threatItems.slice(0, threatCount), ...startupItems.slice(0, startupCount)];
    } else {
      itemsToProcess = prioritized.slice(0, batchSize);
    }
    ...
    for (const rawItem of itemsToProcess) {
      ...
      // Step A.1: Advisory Legitimacy & Anti-Hoax Screening
      const legitimacy = verifyAdvisoryLegitimacy(rawItem);
      if (legitimacy.quarantined) {
        appendAgentLog("decision", `QUARANTINED ADVISORY [Score: ${legitimacy.legitimacyScore}/100, Verdict: ${legitimacy.verdict}]: "${rawItem.title}" - Reason: ${legitimacy.reasons.join('; ') || 'Failed authority audit'}`);
        continue;
      }
      
      // Step B: CISO AI Synthesis & Strategic Rewrite
      appendAgentLog("audit", `Synthesizing CISO executive briefing for '${rawItem.title}' [Provider: ${rawItem.providerName}, Legitimacy: ${legitimacy.legitimacyScore}%, Severity: ${rawItem.severity}, CVE: ${rawItem.cve || 'N/A'}]...`);
      ...
      if (state.autoPublish && isSafe && itemRank >= minRank) {
        ...
        const publishedArt = addPublishedArticle({
          ...
          imageUrl: rawItem.imageUrl || rawItem.thumbnail || ""
        });
        ...
        // Step E: Auto-Syndicate to X
        try {
          const xBroadcast = await publishToX(publishedArt);
          appendAgentLog("info", `Auto-syndicated to X (${xBroadcast.handle}): "${publishedArt.title}" [${xBroadcast.status.toUpperCase()}]`);
        } catch (xErr) {
