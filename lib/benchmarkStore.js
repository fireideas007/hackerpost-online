/**
 * Authoritative AI Security Model Benchmark Registry.
 * Sourced from trusted cybersecurity benchmarking entities:
 * - Meta AI CyberSecEval 3 (Vulnerability exploitation, prompt injection resistance)
 * - MITRE Engenuity Cyber AI Evaluations (Threat hunting & ATT&CK mapping)
 * - USENIX Security SEC-bench (Vulnerability patching & exploit analysis)
 * - Stanford Center for AI Safety (CAIS)
...
export const AI_SECURITY_MODELS = [
  {
...
export function getBenchmarkModels(filterType = "all") {
  if (!filterType || filterType === "all") {
    return AI_SECURITY_MODELS;
  }
  return AI_SECURITY_MODELS.filter(m => m.type.toLowerCase().includes(filterType.toLowerCase()));
}

export function getBenchmarkModelById(id) {
  return AI_SECURITY_MODELS.find(m => m.id === id);
}

export function getBenchmarkEntities() {
  return BENCHMARK_ENTITIES;
}
