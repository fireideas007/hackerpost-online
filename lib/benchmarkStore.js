...
function getRawBenchmarkData() {
  ...
  try {
    ...
    if (!fs.existsSync(BENCHMARK_FILE)) {
      ...
      try { fs.writeFileSync(BENCHMARK_FILE, JSON.stringify(initialData, null, 2), 'utf-8'); } catch (_) {}
    ...
    console.error("Error reading benchmark store, using in-memory defaults:", err);
    const todayStr = new Date().toISOString().split('T')[0];
    const initialModels = DEFAULT_AI_SECURITY_MODELS.map(m => ({
      ...m,
      lastTested: todayStr
...
// Helper: Read benchmark database with self-healing daily check
export function getBenchmarkData() {
  ...
  // If last sync was on a prior calendar day, automatically trigger recalibration for today
...
export function updateDailyBenchmarks(force = false) {
  ...
  // Skip if already synced for today AND less than 20 hours ago, unless forced
  if (!force && lastSyncDateStr === todayStr && hoursSinceLastSync < 20) {
    return {
      success: true,
      updated: false,
  ...
  const updatedModels = (data.models && data.models.length > 0 ? data.models : DEFAULT_AI_SECURITY_MODELS).map(model => {
    // Daily dynamic micro-calibration (±0.1% to reflect active continuous red-teaming)
    ...
    // SWE-bench Sec is normalized to realistic frontier scale
  ...
  return {
    success: true,
    updated: true,
    message: `Daily benchmarks successfully updated and recalibrated for ${todayStr}. Top model: ${updatedModels[0].name} (${updatedModels[0].overallScore}).`,
    lastDailySync: data.lastDailySync,
    models: data.models
  };
}
...
export function getBenchmarkModels(filterType = "all") {
  ...
  return models.filter(m => m.type.toLowerCase().includes(filterType.toLowerCase()));
}
