...
import { getUserById, revokeApiKey } from "@/lib/userStore";
...
export async function POST(request) {
  try {
    ...
    // IDOR Protection: revokeApiKey will throw if keyId is not owned by user.id
    revokeApiKey(user.id, keyId, clientIp);

    return NextResponse.json({
      success: true,
      message: `API Key ${keyId} has been revoked and can no longer be used.`
    });
  } catch (err) {
