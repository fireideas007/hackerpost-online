...
export default function B2bDeveloperPortal() {
  const [apiKey, setApiKey] = useState("hl_live_a7f98e0b2d3c4e5f6a7b8c9d0e1f");
  ...
  const [inputContent, setInputContent] = useState(
    "Notice is hereby given that the Department of Public Works will initiate critical water main repairs on Broadway Avenue. The entire segment will be closed to all vehicular traffic."
  );
  const [inputTitle, setInputTitle] = useState("Water Main Repairs Scheduled");
  const [inputLocation, setInputLocation] = useState("90210");
  const [inputSource, setInputSource] = useState("City DPW Feed");
  ...
  const executePlaygroundRequest = async () => {
    ...
    const payload = activeEndpoint === "audit" 
      ...
      : { 
          ...
          sourceUrl: "https://verifiedwire.org/feed/101" 
        };
  ...
  const getCurlSnippet = () => {
    if (activeEndpoint === "audit") {
      return `curl -X POST https://api.hyperlocal.ai/v1/audit \\
  ...
  -d '{
    "content": "${inputContent.substring(0, 50)}..."
      ...
      return `curl -X POST https://api.hyperlocal.ai/v1/rewrite \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer ${apiKey}" \\
  -d '{
    "title": "${inputTitle}",
  ...
  const getJsSnippet = () => {
    if (activeEndpoint === "audit") {
      return `fetch("https://api.hyperlocal.ai/v1/audit", {
  method: "POST",
  headers: {
...
.then(data => console.log(data));`;
      ...
      return `fetch("https://api.hyperlocal.ai/v1/rewrite", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "Authorization": "Bearer ${apiKey}"
...
url = "https://api.hyperlocal.ai/v1/audit"
headers = {
    "Authorization": "Bearer ${apiKey}",
    "Content-Type": "application/json"
}
...
url = "https://api.hyperlocal.ai/v1/rewrite"
headers = {
    "Authorization": "Bearer ${apiKey}",
    "Content-Type": "application/json"
}
...
print(response.json())`;
  ...
  return (
    <div className="container" style={{ paddingBottom: "100px", paddingTop: "40px" }}>
      {/* Page Hero */}
      <section style={{ textAlign: "center", marginBottom: "48px", background: "radial-gradient(ellipse at top, hsla(var(--primary), 0.08), transparent 70%)", padding: "40px 0", borderRadius: "var(--radius-lg)" }}>
        <span style={{ fontSize: "12px", fontWeight: 700, textTransform: "uppercase", background: "hsla(var(--primary), 0.1)", color: "hsl(var(--primary))", padding: "6px 12px", borderRadius: "20px", marginBottom: "16px", display: "inline-block" }}>
          B2B Developer Solutions
        </span>
        <h1 style={{ fontSize: "36px", fontWeight: 800, letterSpacing: "-1px", marginBottom: "16px" }}>
          Hyperlocal Verification & AI Syndication APIs
        </h1>
        <p style={{ maxWidth: "720px", margin: "0 auto", color: "hsl(var(--muted-foreground))", fontSize: "16px", lineHeight: 1.6 }}>
          Supercharge your newsroom or regional portal. Programmatically verify facts, audit copy-paste plagiarism, localize raw wires, and automate JSON-LD structured schema generations via our low-latency developer endpoints.
        </p>
      </section>

      {/* Upper Grid: API Key Rotator & Pricing Tiers */}
        ...
        <div className="admin-panel" style={{ justifySelf: "stretch" }}>
          ...
          <div className="panel-body" style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <p style={{ fontSize: "13px", color: "hsl(var(--muted-foreground))" }}>
              Include this private Bearer Key in the HTTP request headers (<code style={{ background: "hsl(var(--muted))", padding: "2px 4px", borderRadius: "3px" }}>Authorization: Bearer KEY</code>) to authorize B2B calls.
            </p>
            ...
            <div style={{ display: "flex", gap: "8px", background: "hsl(var(--background))", border: "1px solid hsl(var(--border))", padding: "10px", borderRadius: "var(--radius-sm)", alignItems: "center" }}>
              ...
              <input
                ...
                value={apiKey}
                readOnly
                style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: "13px", fontFamily: "monospace", letterSpacing: "1px" }}
              />
              <button 
            ...
            <div style={{ borderTop: "1px solid hsl(var(--border))", paddingTop: "12px", display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", color: "hsl(var(--success))" }}>
              <ShieldCheck size={16} />
              <span>Authorization check passed: Active Sandbox environment.</span>
            </div>
          </div>
        </div>
          ...
          <div className="panel-body" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
            ...
            <div 
              ...
              style={{
                ...
                borderRadius: "var(--radius-md)", padding: "16px", cursor: "pointer", transition: "all 0.2s"
              }}
            >
              ...
              <ul style={{ paddingLeft: "0", listStyle: "none", fontSize: "11px", color: "hsl(var(--muted-foreground))", display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <li>✓ Jaccard Trigram check</li>
                <li>✓ Standard AI rewriting</li>
                <li>✗ SLA support</li>
              </ul>
            </div>
            ...
            >
              <span style={{ position: "absolute", top: "-10px", right: "10px", background: "hsl(var(--primary))", color: "white", fontSize: "9px", padding: "2px 6px", borderRadius: "10px", fontWeight: 700 }}>
                POPULAR
              </span>
              <h3 style={{ fontSize: "14px", fontWeight: 700 }}>Newsroom Pro</h3>
              ...
              <ul style={{ paddingLeft: "0", listStyle: "none", fontSize: "11px", color: "hsl(var(--muted-foreground))", display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <li>✓ Localized AI rephrasing</li>
                <li>✓ Schema & SEO exports</li>
              </ul>
            </div>
            ...
            >
              ...
              <ul style={{ paddingLeft: "0", listStyle: "none", fontSize: "11px", color: "hsl(var(--muted-foreground))", display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <li>✓ Private LLM models</li>
                <li>✓ Live RSS syncing</li>
                <li>✓ Dedicated 99.9% SLA</li>
              </ul>
            </div>
      ...
      {/* Developer API Console (Playground + Code snippets) */}
      <div className="admin-panel" style={{ maxHeight: "none", display: "flex", flexDirection: "column" }}>
        ...
        <div className="panel-header" style={{ borderBottom: "1px solid hsl(var(--border))" }}>
          <h2 className="panel-title">
            ...
            B2B API Playground & Integration
          </h2>
          <div style={{ display: "flex", gap: "4px", background: "hsl(var(--muted))", padding: "4px", borderRadius: "8px" }}>
            <button
              onClick={() => { setActiveEndpoint("audit"); setApiResponse(null); }}
              className={`filter-btn ${activeEndpoint === "audit" ? "active" : ""}`}
              style={{ margin: 0, padding: "4px 12px", borderRadius: "6px", fontSize: "12px" }}
            >
              POST /v1/audit
            </button>
            <button
            ...
            {activeEndpoint === "rewrite" && (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                  <div>
                    <label className="sandbox-label">Target Locale</label>
                    <input 
                      type="text" 
                      value={inputLocation} 
            ...
            <div style={{ marginTop: "32px" }}>
                  ...
                  {["curl", "js", "python"].map((tab) => (
                    <button
                      ...
                      style={{
                        ...
                        color: activeSnippetTab === tab ? "white" : "hsl(var(--muted-foreground))"
                      }}
                    >
              ...
              <pre 
                style={{
                  ...
                  fontSize: "11px", fontFamily: "monospace", overflowX: "auto", color: "hsl(var(--foreground))", whiteSpace: "pre-wrap"
                }}
              >
          ...
          <div style={{ borderLeft: "1px solid hsl(var(--border))", paddingLeft: "24px", display: "flex", flexDirection: "column", height: "100%" }}>
            ...
            <div 
              style={{
                ...
                padding: "16px", fontFamily: "monospace", fontSize: "11px", overflowY: "auto", position: "relative",
            ...
            >
              {loading ? (
                <div className="flex-center sandbox-loading-pulse" style={{ flex: 1, flexDirection: "column", gap: "10px" }}>
                  ...
                  <span style={{ fontWeight: 600 }}>Awaiting server response...</span>
                </div>
              ) : apiResponse ? (
